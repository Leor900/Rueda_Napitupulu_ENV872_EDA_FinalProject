...
map_migration_by_state_2013 <- plot_usmap(data = data.map_migration_by_state_2013, values = "Prop.Migrants.outflows", labels = TRUE, label_color = "white") + scale_fill_continuous(type = "viridis", name = "Percentage", breaks = c(2, 4, 6, 8), limits = c(0, 10)) + theme(legend.position = "right", plot.title = element_text(hjust = 0.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5))
...
migration_unhealthy_2013_2020 <- AQI_Mig.outflows_by.state_2013_2020 %>% ggplot(aes(x=Migrants.outflows, y=Avg.Unhealthy.Days)) + geom_point() + mytheme + geom_smooth(method = "lm", se = FALSE, color = "black")

# Plot of the relationship between migration and max AQI

migration_AQI_2013_2020 <- AQI_Mig.outflows_by.state_2013_2020 %>% ggplot(aes(x=Migrants.outflows, y=Avg.Max.AQI)) + geom_point() + mytheme + geom_smooth(method = "lm", se = FALSE, color = "black")

# Plot of the relationship between migration and days PM2.5

migration_PM2.5_2013_2020 <- AQI_Mig.outflows_by.state_2013_2020 %>% ggplot(aes(x=Migrants.outflows, y=Avg.Days.PM2.5)) + geom_point() + mytheme + geom_smooth(method = "lm", se = FALSE, color = "black")
...
```{r fig9, echo=FALSE, fig.cap=" Relationship between air quality indicators and migrantion outflows between 2013 and 2020"}
...
cat(' Table: \\label{tab:table2}Correlation between air quality indicators and migration outflows 2013-2020
                                                                |  Confidence Interval  |
                                                                |         95%           |
|Correlations        | Migrants Outflows  |    t    | p-value   |   Min     |  Max      |
|------------------- |--------------------|---------|-----------|-----------|-----------|
|Avg. unhealthy days |      0.2921202     |  6.0936 | 2.609e-09 | 0.1997911 | 0.3793075 |
|Avg. maximum AQI    |      0.2586106     |  5.341  | 1.558e-07 | 0.1647361 | 0.3478420 | |Avg. days PM2.5     |     -0.02109288    | -0.4209 |   0.6741  | -0.118899 | 0.07711834|')
